...
void Queue::del_copy()
{
    ...
    {
        Man *curr, *t, *prev; //создаем вспомогательные указатели на ячейки списка
        for (curr = head; curr != NULL; curr = curr->get_next()) //двигаемся по всей очереди
        {
            prev = curr; //запоминаем сравниваемый элемент
            for (t = curr->get_next(); t != NULL;) //цикл по элементам начиная со следующего от curr
            {
                if (t->get_value() == curr->get_value()) //сравниваем curr со всеми остальными элементами очереди
                {
                    prev->set_next(t->get_next()); //предыдущий указывает теперь на через одного
                    delete t; //удаляем дубликат 
                    count--;
                    t = prev->get_next(); //для того чтобы сравнивать дальше
                    continue; //пропуск оставшейся части кода тела цикла и переход к следующей итерации 
                }
                prev = t; //предыдущий к сравниваемому
                t = t->get_next(); //двигаемся дальше по очереди
            }
        }
